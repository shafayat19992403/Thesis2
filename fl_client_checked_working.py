...
class FlowerClient(fl.client.NumPyClient):
    ...
    #     # Extract images and labels from trainloader with the targeted label
    #     targeted_images = []
    ...
    #     plt.title('PCA of CNN Features for Targeted Label')
    ...
    def fit(self, parameters, config):
        ...
        triggered_indices = np.where(labels == smallest_cluster_label)[0]

        # Check how many of the flagged samples are actually triggered
        actual_triggered_indices = set(triggered_indices) & set(triggered_indices)  # Assuming you have actual triggered indices
        num_actual_triggered_found = len(actual_triggered_indices)

        print(f"Number of actual triggered samples found: {num_actual_triggered_found} out of {len(triggered_indices)}")

        # Plot PCA or other visualizations as needed...
