...
def apply_pca(data, n_components=2):
    # Flatten the images if they are 3D (e.g., [batch_size, height, width])
...
def plot_pca_reduced_data(reduced_data, labels, title='PCA Visualization'):
    plt.figure(figsize=(8, 6))
    scatter = plt.scatter(reduced_data[:, 0], reduced_data[:, 1], c=labels, cmap='viridis', alpha=0.5)
...
# Extract data and labels from the trainloader
...
train_data, train_labels = extract_data_and_labels(trainloader)

# Apply PCA
reduced_train_data = apply_pca(train_data)

# Plot PCA results
plot_pca_reduced_data(reduced_train_data, train_labels)
