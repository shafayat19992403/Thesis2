...
def get_top_weight_contributions(model, image, top_n=10):
    # Step 1: Create a model to get layer outputs and weights
    layer_outputs = []
    weights = []

    # Accessing layers from the Net model
    print(f'model children:{model.children()}')
    for name, layer in model.named_children():  # Use named_children to get layer names
        layer_outputs.append(layer)  # Directly append the layer
        # Ensure the layer has a weight attribute
        if isinstance(layer, (nn.Conv2d, nn.Linear)):
            weights.append(layer.weight.data.cpu().numpy())  # Get weights as numpy array
        else:
            weights.append(None)  # Append None for layers without weights

    # Step 2: Get the activations for the input image
    ...
    with torch.no_grad():  # Disable gradient calculation
        ...
        for layer in model.children():  # Forward pass through each layer
            activations = layer(activations)
            layer_outputs.append(activations)  # Store the activation output

    # Step 3: Calculate weighted contributions for each layer
    weight_contributions = []
    for layer_idx, (weight, activation) in enumerate(zip(weights, layer_outputs[1:])):  # Skip input layer
        if weight is not None and activation is not None:  # Ensure the layer has weights and activations
            # Flatten the activations while keeping the batch size
            batch_size = activation.size(0)
            activation_flat = activation.view(batch_size, -1)  # Flatten activation to (batch_size, num_features)

            # Ensure weight is compatible with the activation shape
            if layer_idx == 0:  # For Conv2D
                num_filters = weight.shape[0]  # Number of output filters
                # Calculate contributions for each filter
                contribution = np.abs(weight) * np.abs(activation_flat.unsqueeze(1))  # Shape: (num_filters, batch_size, height * width)
                contribution = contribution.sum(axis=2)  # Sum over spatial dimensions to get contributions per filter
                weight_contributions.append(contribution)
            elif layer_idx == 1:  # For Linear layer
                contribution = np.abs(weight) * np.abs(activation_flat)
                weight_contributions.append(contribution)

    # Step 4: Flatten and rank contributions
    flat_contributions = np.concatenate([contrib.flatten() for contrib in weight_contributions if contrib is not None])
    sorted_indices = np.argsort(flat_contributions)[::-1]  # Sort by contribution

    # Step 5: Get the top weights and their corresponding contributions
    top_weights = flat_contributions[sorted_indices][:top_n]  # Get top N weights
    top_weight_indices = sorted_indices[:top_n]  # Get indices of top weights

    # Print the top weights and their contributions
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        for images ,labels in trainloader:
            ...
            get_top_weight_contributions(net, image, 100)
            print(net)
            break
